...
/*
   ...
   - To send gyro data on 02
   - When sending 02 from nRF connect, the LED (if connected) turns-on on heel-strike events and turns-off according to VIBRATION_END_PERCENTAGE
...
void sensorTask() {
  ...
  if (currentMillis - previousMillis >= SENSOR_SAMPLING_INTERVAL) {
    ...
    sensorDataUpdated = true;  //Sending angular velocity data on 02, "Record".
    ...
    if (ZeroCrossP2N == true) {
      if (preVal < curVal) {  //After mid-swing and slope goes from positive to negative (ZeroCrossP2N true) we know a heel-strike is coming. When the preVal is smaller than curVal, heel strike has just happened on the prev sample.
        ...
        if (firstHS) {
          firstMillis = millis();
          firstHS = false;
        } else if (secondHS && firstMillis - newMillis > 350) {  //Ensures that first point added to average is not too short
          newMillis = millis();
