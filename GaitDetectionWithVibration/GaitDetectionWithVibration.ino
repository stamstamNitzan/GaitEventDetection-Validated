...
/*
   ...
   - When sending 02 from nRF connect, only the motors are working on heel-strike events and turn-off according to VIBRATION_END_PERCENTAGE
...
#define MOTOR_LED D6  //Leave for a time you want to show the visual LED of HS
#define RED 22
#define BLUE 24
#define GREEN 23
#define LED_PWR 25
...
void setup() {
  ...
  // Vibration motors setup
  vibrationMotorSetup(5);  //real-time mode

  analogReadResolution(12);  //analog read not even used DELETE?
...
void loop() {
  bleTask();
  sensorTask();
  sendDataTask(RECORDING_LENGTH_SECONDS);  //UPDATE13/12/2021:This task is relevant only when we want to resend the data AKA REPLAY.
  vibrationTask();
}
...
void vibrationTask() {
  if (readyToVibrate) {
    ...
    if (currVibeMillis - prevVibeMillis >= stepLengthAvg.get() * VIBRATION_START_PERCENTAGE_MOTOR1 * 0.01) {  //Delay before vibration starts.
      ...
      MotorSelect(FIRST_DRV);    //I don't know why but this line has to stay for the two motors to work independently, even when motor 1 is not connected to multiplexer.02.08.2022
      drv.setRealtimeValue(60);  //0x3A gives 1.5 volt output, 0x3A=58. 100 gives 2.5 volt output, when drv.selectLibrary(1). This control the output voltage to the motors from the drv.
                                 //During the experiments we used 58 but we should use something else with the red motors.
      //Serial.println("Motor 1 on");
      digitalWrite(MOTOR_LED, HIGH);  //LED on
    }

    if (currVibeMillis - prevVibeMillis >= stepLengthAvg.get() * VIBRATION_START_PERCENTAGE_MOTOR2 * 0.01) {  //Vibration starts based on VIBRATION_START_PERCENTAGE_MOTOR2 added 02.08.2022
      MotorSelect(SECOND_DRV);
      drv.setRealtimeValue(60);  //The value here changes the intensity of the first motor as well. Meaning that it doesn't matter which intensity...
                                 // is chosen for the first motor. When this statement is true, both motors will work at this intensity.
      ...
      if (preVal < curVal) {  //After mid-swing and slope goes from positive to negative (ZeroCrossP2N true) we know a heel-strike is coming. When the preVal is smaller than curVal, heel-strike has just happened on the prev sample.
        ...
        if (firstHS) {
          ...
          if (newMillis - oldMillis <= STEP_LENGTH_THRESHOLD * stepLengthAverage) {  //step must be under 150% of prev step to updated step length avg
            ...
            //Serial.println("Step length outside of allowable threshold");
          }
        ...
        //Serial.println("HeelStrikeDetect prevVibeMillis: ");
        //Serial.println(prevVibeMillis);
        //Serial.print("Moving Average: ");
        //Serial.println(stepLengthAvg.get());
      }
    }
  }
}

//Selects multiplexer channel to use
void MotorSelect(uint8_t i) {
  if (i > 7) return;
...
void vibrationMotorSetup(int mode) {  //Mode 5 is Realtime, mode 0 is presets, enters once during setup.
  MotorSelect(FIRST_DRV);             //I don't know why but this line has to stay for the two motors to work independently, even when motor 1 is not connected to the multiplexer.02.08.2022
  drv.begin();
  drv.selectLibrary(2);  //#1 has higher voltage interval control but break time is slower than #2. Read 8.3.5.2 Internal Memory Interface in DRV2605L Datasheet
  drv.setMode(mode);
  MotorSelect(SECOND_DRV);
  drv.begin();
  drv.selectLibrary(2);
